  ...
  public class BattleController : ControllerBase
  {
    ...
    private async Task Fight(User attacker, User opponent, BattleResult result)
    {
      var attackerArmy = await _context.UserUnits
        .Where(usr => usr.UserId == attacker.Id && usr.Health > 0)
        .Include(usr => usr.Unit)
        .ToListAsync();

      var opponentArmy = await _context.UserUnits
        .Where(usr => usr.UserId == opponent.Id && usr.Health > 0)
        .Include(usr => usr.Unit)
        .ToListAsync();
    ...
    private int FightRound(User attacker, User opponent, List<UserUnit> attackerArmy, List<UserUnit> opponentArmy, BattleResult result)
    {
      ...
      if (damage <= randomOpponent.Health)
      {
        randomOpponent.Health -= damage;
        result.Log.Add($"{attacker.Username}'s {randomAttacker.Unit.Title} attacks {opponent.Username}'s {randomOpponent.Unit.Title} with {damage} damage.");
        return damage;
      }
      else
      {
        damage = randomOpponent.Health;
        randomOpponent.Health = 0;
        opponentArmy.Remove(randomOpponent);
        result.Log.Add($"{attacker.Username}'s {randomAttacker.Unit.Title} kills {opponent.Username}'s {randomOpponent.Unit.Title} with {damage} damage.");
        return damage;
      }
    }
